# -*- coding: utf-8 -*-
"""Topic 10 — Areas and the area method."""

FIG_COMMONH = r'''<svg viewBox="0 0 400 260" role="img" aria-label="Two triangles with a common apex and bases on one line">
...
FIG_COMMONA = r'''<svg viewBox="0 0 400 260" role="img" aria-label="Two triangles sharing an angle">
...
FIG_MED6 = r'''<svg viewBox="0 0 400 260" role="img" aria-label="The three medians cut a triangle into six equal-area pieces">
...
FIG_TRAP = r'''<svg viewBox="0 0 400 260" role="img" aria-label="A trapezium and its diagonals">
...
# --- lattice figure for Pick's formula, generated rather than typed by hand ---
_S, _M = 55, 30
_NX, _NY = 6, 5
_W, _H = _M * 2 + _S * _NX, _M * 2 + _S * _NY
def _px(x): return _M + x * _S
def _py(y): return _H - _M - y * _S
_POLY = [(1, 1), (5, 1), (6, 3), (3, 5), (1, 4)]
_BOUND = {(1,1),(2,1),(3,1),(4,1),(5,1),(6,3),(3,5),(1,4),(1,3),(1,2)}
_dots = ''.join(
    '<circle class="%s" cx="%d" cy="%d" r="%s"/>' % (
        ('ptc' if (x, y) in _BOUND else 'pt'), _px(x), _py(y),
        ('3.6' if (x, y) in _BOUND else '2.2'))
    for x in range(_NX + 1) for y in range(_NY + 1))
FIG_PICK = ('<svg viewBox="0 0 %d %d" role="img" aria-label="A lattice polygon for Pick formula">'
            '<path class="lnc" d="M %s Z"/>%s</svg>') % (
    _W, _H,
    ' L '.join('%d %d' % (_px(x), _py(y)) for x, y in _POLY),
    _dots)

LESSON = {
 'n': 10,
 'cat': 'geo',
 'title': {'ru': 'Площади и метод площадей', 'en': 'Areas and the area method'},
 'sub': {'ru': 'Формулы площади, отношения площадей как главный инструмент, теорема Чевы и формула Пика.',
         'en': 'Area formulas, ratios of areas as the main tool, Ceva’s theorem and Pick’s formula.'},
 'goals': {
  'ru': [
   r'Знать формулы площади треугольника, параллелограмма, трапеции и формулу Герона.',
   r'Находить отношения площадей через общую высоту и через общий угол.',
   r'Доказывать геометрические утверждения методом площадей, включая теорему Чевы.',
   r'Считать площадь многоугольника на клетчатой бумаге по формуле Пика.',
  ],
  'en': [
   r'Know the area formulas for triangle, parallelogram and trapezium, and Heron’s formula.',
   r'Find ratios of areas using a common height and using a common angle.',
   r'Prove geometric statements by the area method, Ceva’s theorem included.',
   r'Compute the area of a lattice polygon by Pick’s formula.',
  ]},

 'sections': [

  # ------------------------------------------------------------------ 1
  {'h': {'ru': 'Формулы, которые надо знать', 'en': 'The formulas to know'},
   'blocks': [
    {'t': 'table',
     'head': {'ru': ['Фигура', 'Площадь', 'Когда удобно'],
              'en': ['Figure', 'Area', 'When it is handy']},
     'rows': {
      'ru': [
       [r'треугольник', r'$S=\dfrac{a\,h_a}{2}$', r'известны сторона и высота к ней'],
       [r'треугольник', r'$S=\dfrac{1}{2}ab\sin\gamma$', r'известны две стороны и угол между ними'],
       [r'треугольник', r'$S=\sqrt{p(p-a)(p-b)(p-c)}$', r'известны три стороны ($p$ — полупериметр)'],
       [r'треугольник', r'$S=pr$', r'известен радиус вписанной окружности'],
       [r'параллелограмм', r'$S=a\,h_a=ab\sin\gamma$', r'всегда'],
       [r'трапеция', r'$S=\dfrac{a+b}{2}\,h$', r'средняя линия, умноженная на высоту'],
       [r'ромб, любой четырёхугольник', r'$S=\dfrac{1}{2}d_1d_2\sin\varphi$', r'известны диагонали и угол между ними'],
      ],
      'en': [
       [r'triangle', r'$S=\dfrac{a\,h_a}{2}$', r'a side and its altitude are known'],
       [r'triangle', r'$S=\dfrac{1}{2}ab\sin\gamma$', r'two sides and the angle between them'],
       [r'triangle', r'$S=\sqrt{p(p-a)(p-b)(p-c)}$', r'three sides ($p$ is the semiperimeter)'],
       [r'triangle', r'$S=pr$', r'the inradius is known'],
       [r'parallelogram', r'$S=a\,h_a=ab\sin\gamma$', r'always'],
       [r'trapezium', r'$S=\dfrac{a+b}{2}\,h$', r'midline times height'],
       [r'rhombus, any quadrilateral', r'$S=\dfrac{1}{2}d_1d_2\sin\varphi$', r'the diagonals and the angle between them'],
      ]}},

    {'t': 'note',
     'text': {
      'ru': r'Формула $S=\tfrac12d_1d_2\sin\varphi$ верна для **любого** четырёхугольника, диагонали которого пересекаются, — не только для ромба. Для ромба и квадрата $\varphi=90^\circ$, поэтому $S=\tfrac12d_1d_2$.',
      'en': r'The formula $S=\tfrac12d_1d_2\sin\varphi$ holds for **any** quadrilateral whose diagonals meet — not only for a rhombus. For a rhombus or a square $\varphi=90^\circ$, so $S=\tfrac12d_1d_2$.'}},

    {'t': 'ex',
     'q': {'ru': r'Найдите площадь треугольника со сторонами $13$, $14$ и $15$.',
           'en': r'Find the area of a triangle with sides $13$, $14$ and $15$.'},
     'steps': {
      'ru': [r'Полупериметр: $p=\dfrac{13+14+15}{2}=21$.',
             r'По формуле Герона $$S=\sqrt{21\,(21-13)(21-14)(21-15)}=\sqrt{21\cdot8\cdot7\cdot6}.$$',
             r'Считаем под корнем: $21\cdot6=126$, $8\cdot7=56$, и $126\cdot56=7056$.',
             r'$\sqrt{7056}=84$, потому что $84^{2}=7056$.'],
      'en': [r'Semiperimeter: $p=\dfrac{13+14+15}{2}=21$.',
             r'Heron’s formula gives $$S=\sqrt{21\,(21-13)(21-14)(21-15)}=\sqrt{21\cdot8\cdot7\cdot6}.$$',
             r'Under the root: $21\cdot6=126$, $8\cdot7=56$, and $126\cdot56=7056$.',
             r'$\sqrt{7056}=84$, since $84^{2}=7056$.']},
     'ans': {'ru': r'$S=84$.', 'en': r'$S=84$.'}},
   ]},

  # ------------------------------------------------------------------ 2
  {'h': {'ru': 'Отношения площадей — главный инструмент', 'en': 'Ratios of areas — the main tool'},
   'blocks': [
    {'t': 'idea',
     'text': {
      'ru': r'В олимпиадной задаче площадь редко нужно **вычислять**. Гораздо чаще нужно **сравнить** две площади. Для этого хватает двух правил, и оба следуют прямо из формулы $S=\tfrac12ah$.',
      'en': r'In an olympiad problem an area rarely needs to be **computed**. Far more often two areas need to be **compared**. Two rules suffice, and both follow straight from $S=\tfrac12ah$.'}},

    {'t': 'thm',
     'name': {'ru': 'Правило общей высоты', 'en': 'The common-height rule'},
     'text': {
      'ru': r'Если у двух треугольников основания лежат на одной прямой, а вершина общая, то их площади относятся как основания: $$\frac{S_{ABD}}{S_{ADC}}=\frac{BD}{DC}.$$',
      'en': r'If two triangles have their bases on one line and share the opposite vertex, their areas are in the ratio of the bases: $$\frac{S_{ABD}}{S_{ADC}}=\frac{BD}{DC}.$$'}},

    {'t': 'fig', 'svg': FIG_COMMONH,
     'cap': {'ru': 'У треугольников $ABD$ и $ADC$ одна и та же высота $h$ из вершины $A$, поэтому их площади относятся как $BD:DC$.',
             'en': 'The triangles $ABD$ and $ADC$ have the same altitude $h$ from $A$, so their areas are in the ratio $BD:DC$.'}},

    {'t': 'proof',
     'text': {
      'ru': r'Высота из вершины $A$ к прямой $BC$ — одна и та же для обоих треугольников; обозначим её $h$. Тогда $$S_{ABD}=\frac{BD\cdot h}{2},\qquad S_{ADC}=\frac{DC\cdot h}{2},$$ и при делении $h$ сокращается. $\blacksquare$',
      'en': r'The altitude from $A$ to the line $BC$ is the same for both triangles; call it $h$. Then $$S_{ABD}=\frac{BD\cdot h}{2},\qquad S_{ADC}=\frac{DC\cdot h}{2},$$ and dividing cancels $h$. $\blacksquare$'}},

    {'t': 'thm',
     'name': {'ru': 'Правило общего угла', 'en': 'The common-angle rule'},
     'text': {
      'ru': r'Если точки $M$ и $N$ лежат на сторонах $AB$ и $AC$ треугольника $ABC$, то $$\frac{S_{AMN}}{S_{ABC}}=\frac{AM}{AB}\cdot\frac{AN}{AC}.$$',
      'en': r'If $M$ and $N$ lie on the sides $AB$ and $AC$ of a triangle $ABC$, then $$\frac{S_{AMN}}{S_{ABC}}=\frac{AM}{AB}\cdot\frac{AN}{AC}.$$'}},

    {'t': 'fig', 'svg': FIG_COMMONA,
     'cap': {'ru': 'Треугольники $AMN$ и $ABC$ имеют общий угол $A$; отношение площадей равно произведению отношений сторон, прилежащих к этому углу.',
    ...
    {'t': 'proof',
     'text': {
      'ru': r'Воспользуемся формулой $S=\tfrac12\,xy\sin\alpha$, где $x,y$ — стороны при угле $\alpha$: $$\frac{S_{AMN}}{S_{ABC}}=\frac{\frac12\,AM\cdot AN\sin A}{\frac12\,AB\cdot AC\sin A}=\frac{AM}{AB}\cdot\frac{AN}{AC},$$ поскольку $\sin A$ сокращается. $\blacksquare$\n\n*Без синусов* это можно получить в два шага: сначала сравнить $S_{AMN}$ и $S_{ABN}$ (общая вершина $N$, основания на прямой $AB$), потом $S_{ABN}$ и $S_{ABC}$ (общая вершина $B$, основания на прямой $AC$).',
      'en': r'Use $S=\tfrac12\,xy\sin\alpha$ where $x,y$ are the sides at the angle $\alpha$: $$\frac{S_{AMN}}{S_{ABC}}=\frac{\frac12\,AM\cdot AN\sin A}{\frac12\,AB\cdot AC\sin A}=\frac{AM}{AB}\cdot\frac{AN}{AC},$$ since $\sin A$ cancels. $\blacksquare$\n\n*Without sines* the same follows in two steps: compare $S_{AMN}$ with $S_{ABN}$ (common vertex $N$, bases on the line $AB$), then $S_{ABN}$ with $S_{ABC}$ (common vertex $B$, bases on the line $AC$).'}},

    {'t': 'ex',
     'q': {'ru': r'На сторонах $AB$ и $AC$ треугольника $ABC$ отмечены точки $M$ и $N$ так, что $AM:MB=1:2$ и $AN:NC=3:1$. Какую долю площади треугольника $ABC$ составляет площадь треугольника $AMN$?',
           'en': r'Points $M$ and $N$ on the sides $AB$ and $AC$ of a triangle $ABC$ satisfy $AM:MB=1:2$ and $AN:NC=3:1$. What fraction of the area of $ABC$ is the area of $AMN$?'},
     'steps': {
      'ru': [r'Из $AM:MB=1:2$ следует $AM:AB=1:3$.',
             r'Из $AN:NC=3:1$ следует $AN:AC=3:4$.',
             r'По правилу общего угла $$\frac{S_{AMN}}{S_{ABC}}=\frac13\cdot\frac34=\frac14 .$$'],
      'en': [r'From $AM:MB=1:2$ we get $AM:AB=1:3$.',
             r'From $AN:NC=3:1$ we get $AN:AC=3:4$.',
             r'By the common-angle rule $$\frac{S_{AMN}}{S_{ABC}}=\frac13\cdot\frac34=\frac14 .$$']},
     'ans': {'ru': r'Ровно четверть.', 'en': r'Exactly one quarter.'}},

    {'t': 'warn',
     'text': {
      'ru': r'Отношение $AM:MB=1:2$ и отношение $AM:AB=1:2$ — **разные** вещи. В первом случае $AM$ составляет треть стороны, во втором — половину. Половина ошибок в задачах на площади рождается именно здесь: выписывайте отношение к **целой** стороне отдельной строкой.',
      'en': r'The ratio $AM:MB=1:2$ and the ratio $AM:AB=1:2$ are **different** things. In the first, $AM$ is a third of the side; in the second, a half. Half of all area-problem mistakes are born here: write the ratio to the **whole** side on its own line.'}},
   ]},

  # ------------------------------------------------------------------ 3
  {'h': {'ru': 'Метод площадей', 'en': 'The area method'},
   'blocks': [
    {'t': 'idea',
     'text': {
      'ru': r'**Метод площадей** — это доказательство геометрического факта через сравнение площадей. Схема обычно такая: одну и ту же площадь считают двумя способами, либо разбивают фигуру на части и складывают. Метод особенно хорош там, где прямой счёт углов не идёт.',
      'en': r'The **area method** proves a geometric fact by comparing areas. The pattern is usually one of two: compute the same area in two ways, or split a figure into pieces and add. It is at its best where angle chasing stalls.'}},

    {'t': 'thm',
     'name': {'ru': 'Медианы делят треугольник на шесть равновеликих частей', 'en': 'The medians cut a triangle into six equal-area pieces'},
     'text': {
      'ru': r'Три медианы разбивают треугольник на шесть треугольников, площади которых равны между собой и составляют $\tfrac16$ площади исходного.',
      'en': r'The three medians split a triangle into six triangles of equal area, each one sixth of the whole.'}},

    {'t': 'fig', 'svg': FIG_MED6,
     'cap': {'ru': 'Медианы $AM$, $BN$ и $CK$ пересекаются в точке $G$ и разбивают треугольник на шесть равновеликих частей.',
             'en': 'The medians $AM$, $BN$ and $CK$ meet at $G$ and cut the triangle into six pieces of equal area.'}},

    {'t': 'proof',
     'text': {
      'ru': r'Обозначим $S=S_{ABC}$ и пусть $G$ — точка пересечения медиан.\n\n**Шаг 1.** Медиана $AM$ делит треугольник на две части равной площади $\tfrac S2$: у треугольников $ABM$ и $AMC$ основания $BM=MC$ и общая высота из $A$.\n\n**Шаг 2.** В треугольнике $ABM$ отрезок $GM$ отсекает треугольник $BGM$. Треугольники $ABG$ и $BGM$ имеют общую вершину $B$ и основания $AG$ и $GM$ на одной прямой, причём $AG:GM=2:1$. Значит $$S_{ABG}=2\,S_{BGM},$$ и вместе они дают $\tfrac S2$. Отсюда $$S_{BGM}=\frac{S}{6},\qquad S_{ABG}=\frac{S}{3}.$$\n\n**Шаг 3.** То же рассуждение в треугольнике $AMC$ даёт $S_{CGM}=\tfrac S6$, и точно так же для двух других медиан получаются ещё четыре треугольника площади $\tfrac S6$.\n\nШесть частей, каждая площади $\tfrac S6$, в сумме дают $S$ — значит других частей нет. $\blacksquare$',
      'en': r'Write $S=S_{ABC}$ and let $G$ be the centroid.\n\n**Step 1.** The median $AM$ halves the area: the triangles $ABM$ and $AMC$ have equal bases $BM=MC$ and a common altitude from $A$, so each has area $\tfrac S2$.\n\n**Step 2.** Inside $ABM$, the segment $GM$ cuts off the triangle $BGM$. The triangles $ABG$ and $BGM$ share the vertex $B$ and have bases $AG$ and $GM$ on one line with $AG:GM=2:1$. Hence $$S_{ABG}=2\,S_{BGM},$$ and together they make $\tfrac S2$. Therefore $$S_{BGM}=\frac{S}{6},\qquad S_{ABG}=\frac{S}{3}.$$\n\n**Step 3.** The same argument in $AMC$ gives $S_{CGM}=\tfrac S6$, and likewise the other two medians produce four more triangles of area $\tfrac S6$.\n\nSix pieces of area $\tfrac S6$ add up to $S$ — so there are no others. $\blacksquare$'}},

    {'t': 'ex',
     'q': {'ru': r'Диагонали трапеции $ABCD$ (где $BC\parallel AD$) пересекаются в точке $O$. Известно, что $S_{BOC}=4$ и $S_{AOD}=9$. Найдите площадь трапеции.',
           'en': r'The diagonals of a trapezium $ABCD$ (with $BC\parallel AD$) meet at $O$. Given $S_{BOC}=4$ and $S_{AOD}=9$, find the area of the trapezium.'},
     'steps': {
      'ru': [r'Треугольники $BOC$ и $AOD$ подобны: углы при $O$ вертикальные, а $\angle OBC=\angle ODA$ — накрест лежащие при $BC\parallel AD$.',
             r'Отношение площадей равно квадрату коэффициента: $\dfrac49=k^{2}$, значит $k=\dfrac23$ и $\dfrac{OC}{OA}=\dfrac{OB}{OD}=\dfrac23$.',
             r'Теперь боковые треугольники. У $BOC$ и $AOB$ общая вершина $B$, а основания $OC$ и $OA$ лежат на одной прямой, поэтому $$\frac{S_{AOB}}{S_{BOC}}=\frac{OA}{OC}=\frac32\quad\Longrightarrow\quad S_{AOB}=6 .$$',
             r'Точно так же $S_{COD}=6$.',
             r'Складываем все четыре части: $4+9+6+6=25$.'],
      'en': [r'The triangles $BOC$ and $AOD$ are similar: the angles at $O$ are vertical, and $\angle OBC=\angle ODA$ are alternate angles at $BC\parallel AD$.',
             r'The ratio of areas is the square of the ratio of sides: $\dfrac49=k^{2}$, so $k=\dfrac23$ and $\dfrac{OC}{OA}=\dfrac{OB}{OD}=\dfrac23$.',
             r'Now the side triangles. $BOC$ and $AOB$ share the vertex $B$, and the bases $OC$ and $OA$ lie on one line, so $$\frac{S_{AOB}}{S_{BOC}}=\frac{OA}{OC}=\frac32\quad\Longrightarrow\quad S_{AOB}=6 .$$',
             r'In the same way $S_{COD}=6$.',
             r'Adding the four pieces: $4+9+6+6=25$.']},
     'ans': {'ru': r'$S_{ABCD}=25$. *Общая формула:* $S=\left(\sqrt{S_1}+\sqrt{S_2}\right)^{2}$.',
             'en': r'$S_{ABCD}=25$. *General formula:* $S=\left(\sqrt{S_1}+\sqrt{S_2}\right)^{2}$.'}},

    {'t': 'fig', 'svg': FIG_TRAP,
     'cap': {'ru': 'Диагонали трапеции разбивают её на четыре треугольника; два «боковых» всегда равновелики.',
             'en': 'The diagonals cut a trapezium into four triangles; the two "side" ones always have equal area.'}},
   ]},

  # ------------------------------------------------------------------ 4
  {'h': {'ru': 'Теорема Чевы', 'en': 'Ceva’s theorem'},
   'blocks': [
    {'t': 'def',
     'name': {'ru': 'Чевиана', 'en': 'Cevian'},
     'text': {
      'ru': r'**Чевианой** называют отрезок, соединяющий вершину треугольника с точкой на противоположной стороне. Медиана, биссектриса и высота — частные случаи чевианы.',
      'en': r'A **cevian** is a segment from a vertex of a triangle to a point on the opposite side. Medians, bisectors and altitudes are all cevians.'}},

    {'t': 'thm',
     'name': {'ru': 'Теорема Чевы', 'en': 'Ceva’s theorem'},
     'text': {
      'ru': r'Пусть точки $D$, $E$, $F$ лежат на сторонах $BC$, $CA$, $AB$ треугольника $ABC$. Чевианы $AD$, $BE$, $CF$ пересекаются в одной точке тогда и только тогда, когда $$\frac{BD}{DC}\cdot\frac{CE}{EA}\cdot\frac{AF}{FB}=1 .$$',
      'en': r'Let $D$, $E$, $F$ lie on the sides $BC$, $CA$, $AB$ of a triangle $ABC$. The cevians $AD$, $BE$, $CF$ meet at one point if and only if $$\frac{BD}{DC}\cdot\frac{CE}{EA}\cdot\frac{AF}{FB}=1 .$$'}},

    {'t': 'proof',
     'text': {
      'ru': r'**Необходимость.** Пусть чевианы пересекаются в точке $P$.\n\nПрименим правило общей высоты дважды к отношению $BD:DC$. Треугольники $ABD$ и $ACD$ имеют общую вершину $A$, поэтому $$\frac{S_{ABD}}{S_{ACD}}=\frac{BD}{DC}.$$ Треугольники $PBD$ и $PCD$ имеют общую вершину $P$, поэтому $$\frac{S_{PBD}}{S_{PCD}}=\frac{BD}{DC}.$$\n\nЕсли две дроби равны, то и дробь из разностей равна тому же (при ненулевом знаменателе): $$\frac{S_{ABD}-S_{PBD}}{S_{ACD}-S_{PCD}}=\frac{BD}{DC}.$$ Но $S_{ABD}-S_{PBD}=S_{ABP}$ и $S_{ACD}-S_{PCD}=S_{ACP}$, поэтому $$\frac{S_{ABP}}{S_{ACP}}=\frac{BD}{DC}.$$\n\nСовершенно так же $$\frac{S_{BCP}}{S_{BAP}}=\frac{CE}{EA},\qquad \frac{S_{CAP}}{S_{CBP}}=\frac{AF}{FB}.$$\n\nПеремножим все три равенства. Справа получится искомое произведение, а слева каждая площадь встретится один раз в числителе и один раз в знаменателе, так что всё сократится и останется $1$. $\blacksquare$\n\n**Достаточность.** Пусть произведение равно $1$. Проведём чевианы $AD$ и $BE$; они пересекаются в некоторой точке $P$. Проведём через $P$ отрезок $CF^{\ast}$ (где $F^{\ast}$ на $AB$). По доказанной части $$\frac{BD}{DC}\cdot\frac{CE}{EA}\cdot\frac{AF^{\ast}}{F^{\ast}B}=1 .$$ Сравнивая с условием, получаем $\dfrac{AF^{\ast}}{F^{\ast}B}=\dfrac{AF}{FB}$. Но точка на отрезке $AB$ с заданным отношением ровно одна, значит $F^{\ast}=F$, и все три чевианы проходят через $P$. $\blacksquare$',
      'en': r'**Necessity.** Suppose the cevians meet at a point $P$.\n\nApply the common-height rule twice to the ratio $BD:DC$. The triangles $ABD$ and $ACD$ share the vertex $A$, so $$\frac{S_{ABD}}{S_{ACD}}=\frac{BD}{DC}.$$ The triangles $PBD$ and $PCD$ share the vertex $P$, so $$\frac{S_{PBD}}{S_{PCD}}=\frac{BD}{DC}.$$\n\nWhen two fractions are equal, the fraction of their differences equals the same value (the denominator being non-zero): $$\frac{S_{ABD}-S_{PBD}}{S_{ACD}-S_{PCD}}=\frac{BD}{DC}.$$ But $S_{ABD}-S_{PBD}=S_{ABP}$ and $S_{ACD}-S_{PCD}=S_{ACP}$, so $$\frac{S_{ABP}}{S_{ACP}}=\frac{BD}{DC}.$$\n\nIn exactly the same way $$\frac{S_{BCP}}{S_{BAP}}=\frac{CE}{EA},\qquad \frac{S_{CAP}}{S_{CBP}}=\frac{AF}{FB}.$$\n\nMultiply the three equalities. The right side is the required product, while on the left every area occurs once in a numerator and once in a denominator, so everything cancels and $1$ remains. $\blacksquare$\n\n**Sufficiency.** Suppose the product equals $1$. Draw the cevians $AD$ and $BE$; they meet at some point $P$. Draw through $P$ the segment $CF^{\ast}$ (with $F^{\ast}$ on $AB$). By the part just proved, $$\frac{BD}{DC}\cdot\frac{CE}{EA}\cdot\frac{AF^{\ast}}{F^{\ast}B}=1 .$$ Comparing with the hypothesis gives $\dfrac{AF^{\ast}}{F^{\ast}B}=\dfrac{AF}{FB}$. But a segment $AB$ has exactly one point with a given ratio, so $F^{\ast}=F$ and all three cevians pass through $P$. $\blacksquare$'}},

    {'t': 'idea',
     'text': {
      'ru': r'Теорема Чевы мгновенно доказывает три классических факта о пересечении в одной точке. Для **медиан** все три отношения равны $1$. Для **биссектрис** по свойству биссектрисы $$\frac{BD}{DC}\cdot\frac{CE}{EA}\cdot\frac{AF}{FB}=\frac{c}{b}\cdot\frac{a}{c}\cdot\frac{b}{a}=1 .$$ Для **высот** отношения выражаются через тангенсы и тоже дают единицу.',
      'en': r'Ceva’s theorem instantly proves three classical concurrency facts. For the **medians** all three ratios are $1$. For the **bisectors**, the bisector property gives $$\frac{BD}{DC}\cdot\frac{CE}{EA}\cdot\frac{AF}{FB}=\frac{c}{b}\cdot\frac{a}{c}\cdot\frac{b}{a}=1 .$$ For the **altitudes** the ratios come out in terms of tangents and multiply to one as well.'}},
   ]},

  # ------------------------------------------------------------------ 5
  {'h': {'ru': 'Клетчатая бумага и формула Пика', 'en': 'Lattice paper and Pick’s formula'},
   'blocks': [
    {'t': 'thm',
     'name': {'ru': 'Формула Пика', 'en': 'Pick’s formula'},
     'text': {
      'ru': r'Пусть все вершины многоугольника лежат в узлах клетчатой бумаги со стороной клетки $1$. Обозначим через $I$ количество узлов **строго внутри** многоугольника, а через $B$ — количество узлов **на границе**. Тогда $$S=I+\frac{B}{2}-1 .$$',
      'en': r'Let every vertex of a polygon sit on a unit lattice. Write $I$ for the number of lattice points **strictly inside** the polygon and $B$ for the number **on its boundary**. Then $$S=I+\frac{B}{2}-1 .$$'}},

    {'t': 'fig', 'svg': FIG_PICK,
     'cap': {'ru': 'Здесь $B=10$ (жирные узлы на границе) и $I=11$, поэтому $S=11+5-1=15$.',
             'en': 'Here $B=10$ (the bold points on the boundary) and $I=11$, so $S=11+5-1=15$.'}},

    {'t': 'note',
     'text': {
      'ru': r'Считая $B$, не забывайте узлы, лежащие **на сторонах**, а не только вершины. Сторона из точки $(x_1,y_1)$ в $(x_2,y_2)$ содержит ровно $\gcd\left(|x_2-x_1|,\,|y_2-y_1|\right)$ узлов, если считать один из концов. На рисунке нижняя сторона длины $4$ даёт четыре таких узла, левая сторона длины $3$ — три, а три оставшиеся стороны — по одному.',
      'en': r'When counting $B$, remember the points **on the sides**, not just the vertices. A side from $(x_1,y_1)$ to $(x_2,y_2)$ contains exactly $\gcd\left(|x_2-x_1|,\,|y_2-y_1|\right)$ lattice points if one endpoint is counted. In the figure the bottom side of length $4$ contributes four, the left side of length $3$ contributes three, and the other three sides one each.'}},

    {'t': 'idea',
     'text': {
      'ru': r'Формула Пика — самый быстрый способ найти площадь «кривого» многоугольника на клетчатой бумаге: считать надо только точки. Второй способ — **достроение до прямоугольника**: описать вокруг фигуры прямоугольник и вычесть площади угловых прямоугольных треугольников. Полезно уметь оба и сверять ответы.',
      'en': r'Pick’s formula is the fastest way to find the area of an awkward lattice polygon: you only count points. The other way is **completing to a rectangle**: circumscribe a rectangle and subtract the corner right triangles. It pays to know both and check one against the other.'}},

    {'t': 'note',
     'text': {
      'ru': r'Из формулы Пика следует, что площадь любого многоугольника с вершинами в узлах — число, кратное $\tfrac12$. В частности, **правильный треугольник нельзя расположить так, чтобы все три его вершины попали в узлы решётки** — иначе его площадь $\tfrac{\sqrt3}{4}a^{2}$ была бы рациональной, а $a^{2}$ для решётки целое.',
      'en': r'Pick’s formula shows that the area of any lattice polygon is a multiple of $\tfrac12$. In particular, **an equilateral triangle cannot have all three vertices at lattice points** — otherwise its area $\tfrac{\sqrt3}{4}a^{2}$ would be rational, while $a^{2}$ is an integer for a lattice segment.'}},
   ]},
 ],
