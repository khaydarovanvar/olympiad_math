...
window.LESSONS[14] = {
   ...
   "blocks": [
    {
     "t": "p",
     "text": {
      "ru": "Принцип Дирихле никогда не бывает трудным сам по себе. Вся работа — в выборе клеток. Ниже собраны стандартные конструкции клеток и признаки, по которым каждая узнаётся.",
      "en": "The pigeonhole principle is never the hard part. All the work is in choosing the boxes. Below are the standard constructions and the signals that identify each."
     }
    },
    {
     "t": "table",
     ...
     "rows": {
      "ru": [
       [
        "делимость на $n$",
        "остатки $0,1,\\dots,n-1$",
        "сами числа"
       ],
       [
        "две одинаковые цифры / суммы",
        "возможные значения суммы",
        "наборы"
       ],
       [
        "точки в фигуре",
        "части разрезанной фигуры",
        "точки"
       ],
       [
        "длинная десятичная запись",
        "остатки от деления",
        "числа $1,11,111,\\dots$"
       ],
       [
       ...
       [
        "чётность координат",
        "$4$ класса $\\left(\\text{чёт},\\text{нечёт}\\right)$",
        "точки"
       ],
       [
        "сумма известна",
       ...
       [
        "подмножества набора чисел",
        "возможные суммы подмножеств",
        "подмножества"
       ]
      ],
      "en": [
       [
        "divisibility by $n$",
        "the remainders $0,1,\\dots,n-1$",
        "the numbers themselves"
       ],
       [
        "two equal digits or sums",
        "the possible values of the sum",
        "the tuples"
       ],
       [
        "points inside a figure",
        "the pieces of a dissection",
        "the points"
       ],
       [
        "a long decimal expansion",
        "remainders on division",
        "the numbers $1,11,111,\\dots$"
       ],
       [
        "colours and acquaintance",
        "the edge colours",
        "the pairs of vertices"
       ],
       [
        "parity of coordinates",
        "the $4$ classes $\\left(\\text{even},\\text{odd}\\right)$",
        "the points"
       ],
       [
        "a known total",
    ...
    },
    {
     "t": "thm",
     "name": {
      "ru": "Оценка через среднее",
      "en": "The averaging bound"
     },
     "text": {
      "ru": "Если сумма $n$ чисел равна $S$, то хотя бы одно из них не меньше $\\dfrac{S}{n}$ и хотя бы одно не больше $\\dfrac{S}{n}$.",
      "en": "If $n$ numbers add up to $S$ then at least one of them is at least $\\dfrac{S}{n}$ and at least one is at most $\\dfrac{S}{n}$."
     }
    },
    {
     "t": "proof",
     "text": {
      "ru": "Пусть все числа строго меньше $\\dfrac{S}{n}$. Тогда их сумма строго меньше $n\\cdot\\dfrac{S}{n}=S$ — противоречие ✓ Второе утверждение доказывается так же ✓ $\\blacksquare$\n\nЭто и есть «непрерывный» вариант принципа Дирихле: клетки заменены весами, а утверждение «в какой-то клетке не меньше среднего» — тем же по смыслу.",
      "en": "If every number were strictly less than $\\dfrac{S}{n}$ their sum would be strictly less than $n\\cdot\\dfrac{S}{n}=S$ — a contradiction ✓ The second claim is identical ✓ $\\blacksquare$\n\nThis is the \"continuous\" version of pigeonhole: boxes are replaced by weights, and \"some box holds at least the average\" says the same thing."
     }
    },
    {
     "t": "ex",
     "q": {
      "ru": "Из чисел $1,2,\\dots,2n$ выбрано $n+1$ число. Докажите, что среди выбранных найдутся два, одно из которых делится на другое.",
      "en": "From $1,2,\\dots,2n$ we pick $n+1$ numbers. Prove that one of the chosen numbers divides another."
     },
     "steps": {
      "ru": [
       "**Клетки.** Всякое натуральное число единственным образом записывается как $2^{s}\\cdot q$ с нечётным $q$. Нечётных чисел в диапазоне $1,\\dots,2n$ ровно $n$: это $1,3,5,\\dots,2n-1$. Их и возьмём за клетки.",
       "**Предметы.** Каждое из $n+1$ выбранных чисел кладём в клетку своей нечётной части $q$.",
       "Предметов $n+1$, клеток $n$, значит два числа $a=2^{s}q$ и $b=2^{t}q$ имеют одну и ту же нечётную часть.",
       "Пусть $s<t$ (равенство невозможно: числа различны). Тогда $$\\frac{b}{a}=2^{\\,t-s}$$ — целое, то есть $a$ делит $b$ ✓",
       "Оценка точна: набор $n+1,n+2,\\dots,2n$ состоит из $n$ чисел, среди которых ни одно не делит другое (отношение любых двух строго меньше $2$)."
      ],
      "en": [
       "**The boxes.** Every positive integer is uniquely $2^{s}\\cdot q$ with $q$ odd. There are exactly $n$ odd numbers in $1,\\dots,2n$, namely $1,3,5,\\dots,2n-1$. Take these as the boxes.",
       "**The objects.** Put each of the $n+1$ chosen numbers into the box of its odd part $q$.",
       "With $n+1$ objects and $n$ boxes, two chosen numbers $a=2^{s}q$ and $b=2^{t}q$ share an odd part.",
       "Say $s<t$ (equality is impossible, the numbers being distinct). Then $$\\frac{b}{a}=2^{\\,t-s}$$ is an integer, so $a$ divides $b$ ✓",
       "The bound is sharp: the $n$ numbers $n+1,n+2,\\dots,2n$ contain no such pair (the ratio of any two is strictly less than $2$)."
      ]
     },
     "ans": {
      "ru": "Клетки — нечётные части чисел.",
      "en": "The boxes are the odd parts of the numbers."
     }
    },
    {
     "t": "thm",
     "name": {
      "ru": "Бесконечный принцип Дирихле",
      "en": "The infinite pigeonhole principle"
     },
     "text": {
      "ru": "Если бесконечное множество разбито на конечное число классов, то хотя бы один класс бесконечен.",
      "en": "If an infinite set is split into finitely many classes, at least one class is infinite."
     }
    },
    {
     "t": "proof",
     "text": {
      "ru": "Пусть классов $k$ и каждый конечен, скажем класс с номером $i$ содержит $n_i$ элементов. Тогда всё множество содержит $n_1+\\dots+n_k$ элементов — конечное число, что противоречит его бесконечности ✓ $\\blacksquare$",
      "en": "Suppose there are $k$ classes, all finite, the $i$-th with $n_i$ elements. Then the whole set has $n_1+\\dots+n_k$ elements — a finite number, contradicting its infinitude ✓ $\\blacksquare$"
     }
    },
    {
     "t": "idea",
     "text": {
      "ru": "Бесконечная версия — рабочий инструмент в задачах «докажите, что найдётся бесконечно много…». Типичная схема: разбейте бесконечный набор объектов по конечному числу признаков (остаток, цвет, тип), возьмите бесконечный класс и работайте уже внутри него, где все объекты «одинаковы».",
      "en": "The infinite version is the working tool for problems asking to prove that infinitely many objects of some kind exist. The usual scheme: split an infinite family by finitely many features (a remainder, a colour, a type), take an infinite class, and work inside it where all objects \"look the same\"."
     }
    },
    {
     "t": "warn",
     "text": {
      "ru": "Формулируя решение, всегда **явно называйте предметы и клетки** и проверяйте два пункта: предметов строго больше, чем клеток, и каждый предмет попадает ровно в одну клетку. Решение, в котором написано только «по принципу Дирихле найдутся два одинаковых», не засчитывается — из него не видно, о каких клетках речь.",
      "en": "When writing up, always **name the objects and the boxes explicitly** and check two things: there are strictly more objects than boxes, and every object goes into exactly one box. A solution that says only \"by pigeonhole two of them coincide\" earns no marks — it does not say what the boxes were."
     }
    }
   ]
  }
 ...
 "problems": [
  ...
  {
   "src": {
    "ru": "Теорема Эрдёша–Секереша",
    "en": "Erdős–Szekeres",
    "uz": "Erdyosh–Sekeresh teoremasi"
   },
   "lvl": 3,
   "q": {
  ...
  {
   "src": {
    "ru": "Теорема Дирихле о приближениях",
    "en": "Dirichlet approximation",
    "uz": "Dirixlening yaqinlashtirish teoremasi"
   },
   "lvl": 3,
   "q": {
