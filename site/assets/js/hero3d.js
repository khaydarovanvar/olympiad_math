/* Olympiad Mathematics — the 3-D hero scene.
...
(function (w, d) {
  ...
  function mount(host, opt) {
    ...
    var INK = tone('--navy', '#0F1B2A');
    var GOLD = tone('--gold', '#D08A00');
    var CATS = [tone('--nt', '#1E6FB8'), tone('--alg', '#0F7A5E'),
                tone('--geo', '#B0451F'), tone('--comb', '#5B4FC7')];
    ...
    var moons = [
      { g: cube(), a: 0, s: .46, spin: .0042, tilt: .5, c: CATS[0] },
      { g: tetra(), a: TAU * .25, s: .5, spin: -.0055, tilt: -.3, c: CATS[1] },
      { g: octa(), a: TAU * .5, s: .42, spin: .0048, tilt: .8, c: CATS[2] },
      { g: cube(), a: TAU * .75, s: .34, spin: -.0037, tilt: -.7, c: CATS[3] }
    ];
    ...
    function drawShape(g, rx, ry, rz, scale, off, colour, alpha, width) {
      ...
      edges.forEach(function (item) {
        ...
        var t = depth(item.z);
        ctx.strokeStyle = rgba(colour, alpha * dim * (.2 + t * .8) * clearY(pts, item.e));
    ...
    function resize() {
      ...
      cx = narrow ? W * .5 : W * .68;
      cy = narrow ? H * .84 : H * .5;
      unit = Math.min(W, H) * (narrow ? .088 : .112);
      /* on a phone there is no room beside the copy, so the scene drops to a
         faint texture in the bottom strip rather than competing with it */
      dim = narrow ? .5 : 1;
    }
    ...
    function frame() {
      ...
      /* orbit path */
      drawShape(orbitPath, rx + .48, ry * .6, 0, 1, [0, 0, 0], INK, .16, 1);

      /* the core solid */
      drawShape(core, rx, ry, t * .0007, 1.85, [0, 0, 0], INK, .55, 1.5);
      drawShape(core, rx, ry, t * .0007, 1.85 * .52, [0, 0, 0], GOLD, .5, 1.2);
      ...
      glyphs.map(function (g) {
        ...
        .forEach(function (item) {
          ...
          ctx.font = '600 ' + size.toFixed(1) + 'px "Fraunces",Georgia,serif';
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ctx.fillStyle = rgba(item.g.c, a);
